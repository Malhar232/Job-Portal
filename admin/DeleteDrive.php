...
<ul class="dropdown-content" id="user_dropdown" >
  ...
  <nav style="background-color:#4b0082;" role="navigation">
    <div class="nav-wrapper">
      <a data-activates="slide-out" class="button-collapse show-on-" href="#!"><img src="../images/icon.png" width="65"/></a>
...
<?php
    if (isset($_POST['Yes']))
    {
        $delete_company = "DELETE FROM campusdrive WHERE CompanyName = '{$_GET['q']}' AND CollegeName='{$_GET['c']}'";
        $q_delete_company = mysqli_query($connection, $delete_company);
        if ($q_delete_company)
        {
            ?><script>window.location.replace("../admin/CollegeDrive.php")</script>;<?php
        }
        else
        {
